...
def duplicate_check(df, key_col):
    ...
    duplicates = df.groupBy(key_col).count().filter("count > 1")  # group by pyspark
    #df.createOrReplaceTempView('df')                     # it is spark sql
...
# write_output updater report_lib, for report