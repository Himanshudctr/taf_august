...
def count_check(source_df, target_df, key_columns):
    ...
    if source_cnt == target_cnt:
        ...
        write_output(validation_type='count check', status=status,
                     details=f"Count is matching between source and target, Source count is {source_cnt} and target "
                             f"count is {target_cnt}")
        ...
        status = 'FAIL'
        write_output(validation_type='count check', status=status,
                     details=f"Count is not matching between source and target, Source count is {source_cnt} and "
                             f"target count is {target_cnt} and difference is {source_cnt - target_cnt}")
